...
<body class="bg-surface-light text-dark-slate antialiased font-['Poppins',sans-serif] selection:bg-primary-100 selection:text-primary-800">
  ...
  <main>
    <section id="uploader" class="pt-4 pb-12 sm:pt-6 sm:pb-16 overflow-hidden relative">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <!-- E-E-A-T SEO Breadcrumb & Compact Header (Right at Top) -->
        <div class="text-center mb-4">
          <nav aria-label="Breadcrumb" class="mb-2 text-xs text-slate-400 flex items-center justify-center gap-1.5 flex-wrap">
            ...
            <span>&rsaquo;</span>
            <span class="text-primary-700 font-semibold truncate max-w-[220px] sm:max-w-none"><?= htmlspecialchars($pageH1) ?></span>
          </nav>

          <!-- Headline & Trust Pill -->
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-primary-50 text-primary-700 border border-primary-200 mb-2">
            <span class="w-2 h-2 rounded-full bg-action-500 animate-pulse"></span>
            <span>Image Compressor &amp; Converter • 100% In-Browser Privacy</span>
          </div>

          <h1 id="pageH1" class="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-dark-slate leading-tight mb-2">
            <?= htmlspecialchars($pageH1) ?>
          </h1>
          <p id="pageSubhead" class="text-xs sm:text-sm text-dark-body max-w-2xl mx-auto">
        ...
        <div class="bg-surface-card rounded-3xl shadow-card border border-surface-border p-4 sm:p-8 relative">
          ...
          <div id="dropzone" class="dropzone border-2 border-dashed border-primary-300 hover:border-primary-500 rounded-2xl p-6 sm:p-12 text-center transition-all duration-300 bg-primary-50/20 cursor-pointer group">
            ...
            <div class="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center group-hover:scale-105 transition-transform duration-300 shadow-soft">
            ...
            <button type="button" id="selectImagesBtn" class="inline-flex items-center justify-center gap-3 px-6 py-3.5 sm:px-8 sm:py-4 text-sm sm:text-base font-bold text-white bg-action-600 hover:bg-action-700 active:scale-98 rounded-xl shadow-lg shadow-action-600/25 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-action-500/30">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-slate-400">
          ...
          <!-- Settings Panel (Interactive 2-Column Suite) -->
          <div id="settingsPanel" class="mt-6 pt-6 border-t border-surface-border">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-lg bg-primary-600 text-white flex items-center justify-center text-xs font-extrabold shadow-sm shadow-primary-600/30">1</div>
                <div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 bg-slate-50/70 p-4 sm:p-5 rounded-2xl border border-surface-border shadow-soft">
              ...
              <div class="bg-white p-4 rounded-xl border border-surface-border/80 shadow-xs flex flex-col justify-between">
                <div>
                  <div class="flex items-center justify-between mb-3">
                    <label id="labelQuality" class="text-xs font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1.5">
                      <svg class="w-4 h-4 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
                      </svg>
                      <span><?= htmlspecialchars($t['label_quality'] ?? 'Image Quality') ?></span>
                    </label>
              ...
              <!-- Interactive Output Format Cards -->
              <div class="bg-white p-4 rounded-xl border border-surface-border/80 shadow-xs flex flex-col justify-between">
                <div>
                  <div class="flex items-center justify-between mb-3">
                    <label id="labelFormat" class="text-xs font-bold uppercase tracking-wider text-slate-600 flex items-center gap-1.5">
                      <svg class="w-4 h-4 text-action-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      <span><?= htmlspecialchars($t['label_format'] ?? 'Output Format') ?></span>
                    </label>
                    <span class="text-[10px] font-bold text-action-700 bg-action-50 border border-action-200 px-2 py-0.5 rounded-md uppercase">Next-Gen Ready</span>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-2" id="formatCardsGrid">
                    ...
                    <button type="button" data-format="original" class="format-card text-left p-2.5 rounded-xl border transition-all relative <?= $targetFormat === 'original' ? 'border-primary-600 bg-primary-50/70 text-primary-900 shadow-sm ring-2 ring-primary-500/20' : 'border-slate-200 hover:border-slate-300 bg-white text-slate-700' ?>">
                      ...
                      <div class="text-[11px] text-slate-500 leading-tight">Same as source</div>
                    </button>
            ...
            <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div class="flex items-center gap-2.5">
                <button type="button" id="clearAllBtn" class="hidden px-3.5 py-2 text-xs font-semibold text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                  <?= htmlspecialchars($t['btn_clear'] ?? 'Clear All') ?>
                </button>
                <button type="button" id="compressAllBtn" class="px-5 py-2 text-xs sm:text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 rounded-xl shadow-md shadow-primary-600/20 transition-all flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                  <span><?= htmlspecialchars($t['btn_recompress'] ?? 'Re-Compress All') ?></span>
                </button>
                <button type="button" id="downloadAllBtn" class="hidden px-5 py-2 text-xs sm:text-sm font-bold text-white bg-action-600 hover:bg-action-700 rounded-xl shadow-md shadow-action-600/20 transition-all items-center gap-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Batch Metrics & Bulk Download Hub -->
          <div id="batchStatsCard" class="mt-6 hidden bg-gradient-to-r from-action-50/70 via-white to-primary-50/70 border border-action-200/90 rounded-2xl p-4 sm:p-5 shadow-soft">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
              <!-- Summary Numbers -->
              <div class="flex items-center gap-4 sm:gap-6 text-center sm:text-left w-full sm:w-auto justify-around sm:justify-start">
                <div>
                  <div class="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Total Files</div>
                  <div id="statTotalCount" class="text-base sm:text-lg font-black text-dark-slate">0</div>
                </div>
                <div class="border-x border-slate-200 px-3 sm:px-6">
                  <div class="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">File Size</div>
                  <div class="text-xs sm:text-sm font-bold">
                    <span id="statOriginalSize" class="text-slate-400 line-through">0 KB</span>
                    <span class="text-slate-400 mx-1">&rarr;</span>
                    <span id="statNewSize" class="text-primary-700">0 KB</span>
                  </div>
                </div>
                <div>
                  <div class="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Space Saved</div>
                  <div id="statSavingsPercent" class="text-base sm:text-lg font-black text-action-700">&darr; 0%</div>
                </div>
              </div>

              <!-- ZIP Download Action -->
              <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
                <button type="button" id="downloadZipBtn" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 text-xs sm:text-sm font-bold text-white bg-action-600 hover:bg-action-700 active:scale-95 rounded-xl shadow-lg shadow-action-600/30 transition-all">
                  <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
                  <span id="downloadZipBtnText">Download All as ZIP (0 Files)</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div id="resultsContainer" class="mt-8 space-y-4 hidden">
            <div class="flex items-center justify-between">
              <h4 class="text-sm font-bold uppercase tracking-wider text-slate-500">Processed Images (<span id="resultsCount">0</span>)</h4>
              <span class="text-xs text-slate-400">Download individually or all at once</span>
            </div>
            <div id="resultsList" class="grid grid-cols-1 md:grid-cols-2 gap-3.5"></div>
          </div>

        </div>

        <!-- Leaderboard Ad Space (Cleanly Placed Below Tool) -->
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
        ...
        <div class="flex items-center gap-4">
          ...
          <a href="#eeat-authority" class="hover:text-primary-600 transition-colors">Editorial & E-E-A-T Policy</a>
        </div>
      </div>
